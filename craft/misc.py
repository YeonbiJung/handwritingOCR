...
from craft.data_manipulation import two_char_bbox_to_affinity
...
def merge_adjacent_boxes(
    ...
    if direction=="left":
        for i in range(len(boxes1)):
            if i >= len(boxes1): break
            lx = boxes1[i, 0, 0]
            if lx < half + 0.01 * half:
                my = boxes1[i, :, 1].mean().astype(int)  # mean of y
                if affinity_map[my, lx] > threshold:
                    # two boxes should be merged
                    point = (half-1, my)
                    for ii in range(len(boxes2)):
                        if is_in_box(point, boxes2[ii, :, :]):
        ...
        for i in range(len(boxes1)):
            ...
            if ly < half + 0.01 * half:
                mx = boxes1[i, :, 0].mean().astype(int)  # mean of x
    ...
    for i, ii in to_be_merged:
        rbox = boxes1[i, :, :]
        lbox = boxes2[ii, :, :]
        new_box = merge_two_box(rbox, lbox)
